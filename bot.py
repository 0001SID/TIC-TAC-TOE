    ...
    if hit == 0: #If there is no any winning position then check one more level
        for i in blank_positions:
            ...
            if depth == level: #if The depth of finding probability and no any winning position there then return Zero probability.The greater the level the deeper the program will calculate
                ...
                probability = opponent_turn_probability(board_copy,level,depth) #This is the probabilities to win for that X move
                if who == 'bot':
...
def opponent_turn_probability(board,level,depth):
    blank_positions = check_blank_position(board) #Check all the blank position in the board that we have now
    temp = 0
    length = 0
    hit = 0
    for i in blank_positions:
        board_copy = copy.copy(board)
        board_copy[str(i)] = 'O'
        if revolutin(board_copy) == -1: #If we find anny position of winnig for the opponent then there the probability of winning is negetive,and here -100
            probability = -100
            hit = 1
            break
    if hit == 0:
        for i in blank_positions:
            board_copy = copy.copy(board)
            board_copy[str(i)] = 'O'
            temp += myturn_probability(board_copy,depth+1,'opponent',level) #Check the next level for X if there is a situation for X to win if yes probability is 100 other wise calculate
            length += 1
        if temp == 0:
            probability = 0
        else:
            probability = (temp/(length*100))*100 #This is the probability of winning for a X move with all the possible value of O
    return probability
